...
# Remove snap firefox if present (ignore errors)
...
echo "deb https://ppa.launchpadcontent.net/mozillateam/ppa/ubuntu ${codename} main" | tee \
    "/etc/apt/sources.list.d/mozillateam-ubuntu-ppa-${codename}.list" >/dev/null
...
echo "Unattended-Upgrade::Allowed-Origins:: \"LP-PPA-mozillateam:${codename}\";" | \
    tee /etc/apt/apt.conf.d/51unattended-upgrades-firefox >/dev/null
